...
def eval_ewa(
    ...
    if cfg.wandb:
        if curr_iter == cfg.iterations or full_eval:
            logs = {
                "iter": curr_iter,
                f"final-val/loss_{prefix}": val_loss,
                f"final-val/perplexity_{prefix}": val_perplexity,
                f"final-val/acc_{prefix}": val_acc,
            }
        else:
            logs = {
                "iter": curr_iter,
                f"val/loss_{prefix}": val_loss,
                f"val/perplexity_{prefix}": val_perplexity,
                f"val/acc_{prefix}": val_acc,
            }
        if cfg.moe and cfg.plot_router_logits:
            pass
        wandb.log(logs)
    print(
        f">EWA Eval({prefix}): Iter={curr_iter} "
        f"val_loss={val_loss:.3f} "
        f"val_pp={val_perplexity:.3f} "
        f"val_acc={val_acc:3f}"
    )
