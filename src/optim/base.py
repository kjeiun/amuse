...
from .utils import (eval, get_batch, get_parameter_norms, load_checkpoint,
                    load_worker_state, log_prodigy_lr, save_checkpoint, save_schedulefree_xy_models,
...
def _total_grad_norm(parameters, norm_type=2.0):
...
def train(
    ...
    grad_probe_reader = None
    if cfg.grad_probe_interval > 0:
        probe_data_src = (
            train_reader.data if train_reader.data is not None else train_reader.data_path
        )
        grad_probe_reader = DataReader(
            data_src=probe_data_src,
            batch_size=train_reader.batch_size,
            sequence_length=train_reader.sequence_length,
            seed=cfg.grad_probe_data_seed,
            with_replacement=False,
            auto_shard=True,
            keep_in_ram=getattr(train_reader, "keep_in_ram", False),
        )
    ...
    def _run_grad_norm_probe():
        if grad_probe_reader is None or cfg.grad_probe_batches <= 0:
            return None

        probe_norms = []
        grad_probe_reader.set_step(0)
        model.train()
        if is_schedulefree:
            opt.train()

        for probe_idx in range(cfg.grad_probe_batches):
            opt.zero_grad(set_to_none=True)
            x_probe, y_probe = get_batch(grad_probe_reader, device=cfg.device)
            with type_ctx:
                with distributed_backend.get_context_for_microstep_forward(
                    model=model,
                    microstep_idx=probe_idx,
                    gradient_accumulation_steps=1,
                ):
                    probe_outputs = model(x_probe, targets=y_probe, moe=cfg.moe)

            probe_outputs["loss"].backward()
            raw_model = distributed_backend.get_raw_model(model)
            probe_norms.append(_total_grad_norm(raw_model.parameters()))

        opt.zero_grad(set_to_none=True)

        probe_tensor = torch.tensor(probe_norms, dtype=torch.float32)
        return {
            "mean": probe_tensor.mean().item(),
            "var": probe_tensor.var(unbiased=False).item(),
            "std": probe_tensor.std(unbiased=False).item(),
            "min": probe_tensor.min().item(),
            "max": probe_tensor.max().item(),
        }
    model.train()
    ...
    while curr_iter <= cfg.iterations:
        ...
        # Save both x/y models only at selected iterations
        if (is_schedulefree and cfg.save_model):
            _dist_barrier()
            if (distributed_backend.is_master_process() and curr_iter % 2000 == 0):
                save_schedulefree_xy_models(
                    not_compiled_model,
                    opt,
                    curr_iter,
                    exp_dir / "ckpts" / "schedulefree_xy",
                )
            _dist_barrier()
        elif cfg.save_model and curr_iter % 10 == 0:
            _dist_barrier()

            if distributed_backend.is_master_process():
                ckpt_dir = exp_dir / "ckpts" / f"model_{curr_iter}"
        ...
        curr_iter += 1
        grad_probe_stats = None
        if cfg.grad_probe_interval > 0 and curr_iter % cfg.grad_probe_interval == 0:
            grad_probe_stats = _run_grad_norm_probe()
