...
def load_checkpoint(
    ...
    if load_scheduler is None:
        load_scheduler = load_optimizer
    ...
    if load_scheduler and scheduler is not None and ckpt.get("scheduler") is not None:
        try:
            ...
            print(
                "[Resume] skipping scheduler state restore due to incompatible checkpoint "
                f"state: {exc}"
            )

    itr = ckpt.get("itr", -1)
    return itr, ckpt_path.parent, scheduler_loaded

def resolve_checkpoint_path(resume_from):
    if resume_from is None:
        return None

    ckpt_path = Path(resume_from)
    if ckpt_path.is_dir():
        ckpt_path = ckpt_path / "main.pt"
